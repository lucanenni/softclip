"use strict";
/* =====================================================================
   SOURCE MANAGER — mic / demo riff / audio file. Same design as
   softclip's SRC: single owner of engine on/off + input routing.
   ===================================================================== */
const SRC={
  mode:null,stream:null,micNode:null,devId:'',
  riffKit:null,riffTimer:null,riffNext:0,riffStep:0,riffVoices:[],
  fileBuf:null,fileNode:null,devLabel:'',
  ...
  async refreshDeviceList(){
    ...
    try{
      ...
      devs.forEach(d=>{
        ...
        o.value=d.deviceId;o.textContent=d.label||('input '+d.deviceId.slice(0,5));
  ...
  renderKit(){
    ...
    const mk=freq=>{
      ...
      const loss=.9962;
      for(let i=N;i<len;i++)d[i]=loss*(.52*d[i-N]+.52*d[i-N-(i>N?1:0)]);
  ...
  startRiff(){
    ...
    const LEAD=[[0,0,1],[2,12,.9],[4,15,.9],[7,14,.75],[8,12,.95],[10,0,.7],[11,7,.9],[14,5,.85],
      [16,0,1],[18,7,.85],[20,10,.9],[23,12,.75],[24,12,.95],[26,15,.9],[28,17,.85],[30,15,.7]];
    const STRUM={0:{notes:[0,7,12,16],vel:.95},16:{notes:[0,7,12,16],vel:1}};
    const leadByStep={};LEAD.forEach(L=>leadByStep[L[0]]=L);
    const tick=()=>{
      const c=ENG.ctx,spb=60/120/4;
      while(this.riffNext<c.currentTime+.3){
        ...
        const hit=(buf,when,vel,rate)=>{
          ...
          s.start(when);this.riffVoices.push(s);
        };
        const strum=STRUM[st];
        if(strum)strum.notes.forEach((semi,i)=>hit(K.open[0],t+i*.014,strum.vel*(i===3?1:.8),Math.pow(2,semi/12)));
        const ld=leadByStep[st];
        if(ld)hit(K.open[Math.min(5,Math.floor(Math.abs(ld[1])/5))],t,ld[2],Math.pow(2,ld[1]/12));
        this.riffNext+=spb;
        this.riffStep=(this.riffStep+1)%32;
      }
...
function refreshSourceUI(){
  document.querySelectorAll('.srcbtn').forEach(b=>b.classList.toggle('on',b.dataset.src===SRC.mode));
